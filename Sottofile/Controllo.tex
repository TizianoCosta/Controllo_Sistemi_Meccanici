...
Tipicamente le pulsazioni sono legate dalla relazione: \(\omega_a < \frac{K_{pv}K_T}{J} < \omega_{bw}\) e da attraversamento a banda passante può esserci un \(20\% \div 50\%\) di incremento, tuttavia, per semplificazione:
\[\omega_a \simeq \frac{K_{pv}K_T}{J} \simeq \omega_{bw}\]
...
\[
\begin{cases}
    \omega_{bw}^{des} \simeq \frac{\frac{1}{T_{eq}}}{4\xi^2_{des}} \\
    K_{pv} \simeq \frac{\omega_{bw}^{des}J}{K_T} \\
    \frac{1}{T_{iv}} \simeq \frac{\omega_{bw}^{des}}{4\xi^2_{des}}
\end{cases}
\]
...
Trascurando l'anello di corrente e il campionamento\footnote{Se li considerassimo \(\omega_{bw}^{des} < \frac{\omega_{tv}}{4}\).}, per \(\xi_{des}=1\), vale \(T_{eq}=\frac{1}{\omega_{tv}}\), da cui \(\omega_{bw}^{des}\simeq \frac{\omega_{tv}}{4}\).
...
% Immagini relative la misura di una velocità con encoder a risoluzioni differenti
...
Prendendo gli stessi encoder di prima, ma cercando di ottenere \textbf{parità di rumore}, quindi filtrando maggiormente per quello avente minor risoluzione e di conseguenza aumentandone il ritardo, si nota come \(K_{pv}=\frac{\omega_{bw}^{des}J}{K_T}\) ossia \textbf{maggiore la banda passante, maggiore l'amplificazione del rumore}. Nell'esempio precedente i valori sono \(K_{pv}=1.3\) per \(\omega_{bw}=136 \unitamisura{rd}{s}\) e \(K_{pv}=6.6\) per \(\omega_{bw}=660 \unitamisura{rd}{s}\).
...
% Immagini relative la misura di una velocità con encoder a risoluzioni differenti con filtri differenti e come varia il K_pv
...
Tuttavia l'accelerazione non viene misurata direttamente, ma si può ricavare a partire da misure di corrente (che sono influenzate dalla banda passante di corrente, che però è elevata); valgono: \(C_m=K_T i= J \AccAng + C_e\), da cui \(\AccAng = \frac{K_T i - C_e}{J}\) in cui l'unico elemento non noto è la coppia legata alle forze esterne \(C_e\).
...
Così facendo si ottiene una stima a ridotto rumore, ritardo e con \(\omega_{tv}\simeq \frac{f_c}{2}\)

\import{Immagini}{stimatore_velocita}

% aggiungere immagini
% aggiungere in appendice teorema di Shannon e inserire riferimento nel footnote